This directory contains integration tests that test ecurrencyd and its
...
ecurrencyd and ecurrency-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the ecurrency utilities, currently only
ecurrency-tx.
...
The P2P and RPC ports used by the ecurrencyd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another ecurrencyd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its ecurrencyd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other ecurrencyd processes are running.

On linux, the test framework will warn if there is another
ecurrencyd process running when the tests are started.

If there are zombie ecurrencyd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
ecurrencyd processes running on the system, so should not be used if any non-test
ecurrencyd processes are being run.**
...
sure ecurrencyd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and ecurrencyd `debug.log`s will all be dumped
...
`test_framework.log` and ecurrencyd `debug.log`s can be combined into a single
...
call methods that interact with the ecurrencyd nodes-under-test.

If further introspection of the ecurrencyd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/ecurrencyd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/ecurrencyd <pid>
```
