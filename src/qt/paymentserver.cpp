...
const char* BIP71_MIMETYPE_PAYMENT = "application/ecurrency-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/ecurrency-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/ecurrency-paymentrequest";
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start ecurrency: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("ecurrency://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'ecurrency://' is not a valid URI. Use 'ecurrency:' instead."),
    ...
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid eCurrency address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
